...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 270  # +12 note_* tools from the Notes merge; ONE surface (stdio == HTTP)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 270  # +12 note_* tools from the Notes merge; ONE surface (stdio == HTTP)
