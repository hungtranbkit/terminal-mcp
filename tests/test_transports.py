...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 140  # +1 P0.3 terminal_node_capabilities (worker tool/runtime axis); ...previous total (131) +8 Auto Recovery tools (terminal_recovery_set_policy/status/list/loop_status/loop_run_once, terminal_recover_session, terminal_recovery_reconcile_node, terminal_checkpoint_session)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 140  # +1 P0.3 terminal_node_capabilities (worker tool/runtime axis); ...previous total (131) +8 Auto Recovery tools (terminal_recovery_set_policy/status/list/loop_status/loop_run_once, terminal_recover_session, terminal_recovery_reconcile_node, terminal_checkpoint_session)
