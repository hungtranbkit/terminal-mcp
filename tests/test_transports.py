...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 107  # ...previous total (99) +8 terminal_pm_* tools (PM/Orchestrator Agent, sec20.2)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 107  # ...previous total (99) +8 terminal_pm_* tools (PM/Orchestrator Agent, sec20.2)
