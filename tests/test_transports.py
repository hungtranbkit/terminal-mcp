...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 203  # Orchestration V1: +8 outcome layer, +4 worker view; +1 terminal_fleet_environment; ONE surface (stdio == HTTP)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 203  # Orchestration V1: +8 outcome layer, +4 worker view; +1 terminal_fleet_environment; ONE surface (stdio == HTTP)
