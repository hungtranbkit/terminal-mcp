...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 198  # +8 Orchestration V1 outcome layer; ONE surface (stdio == HTTP since build_mcp defaults backlog+events)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 198  # +8 Orchestration V1 outcome layer; ONE surface (stdio == HTTP since build_mcp defaults backlog+events)
