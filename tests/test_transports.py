...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 88  # +terminal_registry_* (5) +terminal_knowledge_* (4) +terminal_watchdog_* (4, session/node drop detection) +terminal_queue_* (15, Supervisor Queue v2 Phase 1+2) +terminal_integration_* (10, 3-role model Integration Agent) +terminal_enqueue_task/terminal_task_status/terminal_queue_metrics (3, P0 persist-before-dispatch) +terminal_task_set_project/reassign/assignment_history/rebalance_plan/rebalance (5, Task Migration/Load Balancing)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 88  # +terminal_registry_* (5) +terminal_knowledge_* (4) +terminal_watchdog_* (4, session/node drop detection) +terminal_queue_* (15, Supervisor Queue v2 Phase 1+2) +terminal_integration_* (10, 3-role model Integration Agent) +terminal_enqueue_task/terminal_task_status/terminal_queue_metrics (3, P0 persist-before-dispatch) +terminal_task_set_project/reassign/assignment_history/rebalance_plan/rebalance (5, Task Migration/Load Balancing)
