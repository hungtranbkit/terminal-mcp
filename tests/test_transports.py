...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 51  # +terminal_registry_* (5, Persistent Session Registry) +terminal_knowledge_* (4, Session Knowledge Store)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 51  # +terminal_registry_* (5, Persistent Session Registry) +terminal_knowledge_* (4, Session Knowledge Store)
