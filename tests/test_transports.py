...
async def test_stdio_real_handshake_and_tools(tmp_path):
    ...
    assert len(names) == 274  # 270 on main + 4 worktree_janitor_* tools; ONE surface (stdio == HTTP)
...
async def test_http_real_handshake_tools_and_security(http_server, tmux_session_factory):
    ...
    assert len(tools.tools) == 280  # includes node-health + compact connection/control tools; ONE surface (stdio == HTTP)
