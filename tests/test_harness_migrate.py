...
def populated(tmp_path):
    ...
    # Reconstruct a genuine v14 file: drop what v15-v18 added and wind the
    ...
    try:
        ...
        connection.execute("ALTER TABLE queue_tasks DROP COLUMN inactive_observed_at")
        connection.execute("ALTER TABLE queue_tasks DROP COLUMN inactive_signature")
...
def test_a_dry_run_reports_what_would_happen_and_changes_nothing(populated):
    ...
    assert report.applied == (15, 16, 17, 18)
...
def test_migrating_adds_the_harness_tables_and_touches_no_existing_row(populated):
    ...
    assert report.version_after == 18
    assert report.applied == (15, 16, 17, 18)
    ...
    with sqlite3.connect(populated) as connection:
        ...
        assert connection.execute(
            "SELECT inactive_observed_at, inactive_signature FROM queue_tasks").fetchall() == [(None, None)] * 6
