...
Marked `browser_smoke`, so it is opt-in exactly like every other real-Chromium
test here: `pytest -m browser_smoke tests/test_dashboard_nav_browser_smoke.py`.

WHY A REAL BROWSER AND NOT MORE STRING ASSERTIONS. The markup tests next door
prove the menu is IN every page. They cannot prove it is VISIBLE -- a bar that
is present but zero-height, painted under a full-height flex child, or
collapsed off-screen inside a CSS grid would pass every one of them and fail
the only thing the user asked for. So this measures the rendered bar on each
page, at each viewport, and then clicks its way around the whole dashboard,
asserting after every hop that it is still there.

It serves the REAL app on loopback rather than stubbing routes: the point is
the pages as assembled, including the ones whose layout the bar had to adapt
to (full-height flex, ordinary flow, and AI Usage's CSS grid).
"""
...
from terminal_mcp.dashboard_nav import DESTINATIONS, NAV_HEIGHT_PX

#: /dashboard/requirements serves the doc as text/plain, so it is a real
#: destination with no bar to measure. Everything else is an HTML page.
PAGES = [d for d in DESTINATIONS if d.renders_nav]
...
#: one-way trip. Ordered as a real operator would walk them.
WALK = ["projects", "agents", "tasks", "work", "sessions", "nodes", "home"]
...
def page(server_url):
    ...
    with sync_playwright() as pw:
        try:
            ...
            # subprocess on a sandboxed host and aborts with "GPU process isn't
            # usable" unless it is kept in one process.
...
def _nav_box(page):
    return page.evaluate("""() => {
      const bar = document.querySelector('.tmcp-global-nav');
      if (!bar) return null;
...
def _goto(page, server_url, href):
    ...
    page.wait_for_selector(".tmcp-global-nav", timeout=15000)
...
def test_the_menu_is_rendered_and_on_screen(page, server_url, destination, viewport):
    ...
    assert box["height"] >= NAV_HEIGHT_PX - 2, (
        ...
        f"markup, invisible on screen, which is the failure this whole pass is about")
...
def test_the_menu_does_not_steal_the_page(page, server_url, destination):
    """Compact means compact: chrome on an operations screen."""
    page.set_viewport_size(LAPTOP)
    _goto(page, server_url, destination.href)
    box = _nav_box(page)
    assert box["height"] / LAPTOP["height"] < 0.08, (
...
def test_a_page_cannot_restyle_the_shared_bar_out_of_shape(page, server_url, destination):
    """The bar is a <nav>, so a page that styles the bare `nav` element wins
    over nothing -- it only loses to a class.

    AI Usage really does this: its own sidebar is a bare `nav` rule with
    `flex-direction:column`, `overflow-y:auto` and a right border. All three
    leaked in, and the bar rendered as two centred rows with its overflow
    panel clipped away. Measured here rather than asserted in CSS, because
    "which rule won" is exactly the thing a stylesheet cannot tell you."""
    page.set_viewport_size(LAPTOP)
    _goto(page, server_url, destination.href)
    shape = page.evaluate("""() => {
      const bar = document.querySelector('.tmcp-global-nav');
      const style = getComputedStyle(bar);
      const more = document.querySelector('#tmcpNavMore');
      return { direction: style.flexDirection, overflow: style.overflowY,
               wrap: style.flexWrap, height: bar.getBoundingClientRect().height,
               moreWidth: more ? more.getBoundingClientRect().width : 0,
               moreVisible: more ? more.getBoundingClientRect().width > 0 : false };
    }""")
    assert shape["direction"] == "row", f"{destination.href}: the bar stacked vertically"
    assert shape["wrap"] == "nowrap", f"{destination.href}: the bar wrapped to a second row"
    assert shape["overflow"] == "visible", (
        f"{destination.href}: overflow is {shape['overflow']!r}, which clips the dropdown")
    assert shape["moreVisible"], f"{destination.href}: the overflow button is not rendered"
    assert shape["moreWidth"] < LAPTOP["width"] * 0.3, (
        f"{destination.href}: the overflow button stretched to {shape['moreWidth']}px")
    assert shape["height"] <= NAV_HEIGHT_PX + 2


def test_the_overflow_panel_opens_on_every_page(page, server_url):
    """The panel is absolutely positioned inside the bar. A page whose own
    styles give the bar `overflow: auto` clips it into a one-line scroller,
    which looks like the menu simply not working."""
    page.set_viewport_size(LAPTOP)
    for destination in PAGES:
        _goto(page, server_url, destination.href)
        page.click("#tmcpNavMore")
        page.wait_for_selector("#tmcpNavPanel.open", timeout=5000)
        height = page.evaluate(
            "() => document.querySelector('#tmcpNavPanel').getBoundingClientRect().height")
        assert height > 100, f"{destination.href}: the panel opened {height}px tall"
...
def test_a_whole_tour_of_the_dashboard_without_pressing_back(page, server_url):
    ...
    using only the menu. Nothing here calls page.go_back()."""
    page.set_viewport_size(LAPTOP)
    _goto(page, server_url, "/dashboard")
    visited = ["/dashboard"]

    for key in WALK:
        destination = next(d for d in DESTINATIONS if d.key == key)
        link = page.locator(f'.tmcp-global-nav .tmcp-nav-links a[href="{destination.href}"]')
...
def test_a_secondary_section_is_one_click_from_a_primary_one(page, server_url):
    """Fleet registry, Audit, AI Usage and the ops screens live in the
    overflow panel. One click to open it, one to arrive -- never Back."""
    page.set_viewport_size(LAPTOP)
    _goto(page, server_url, "/dashboard/projects")
    page.click("#tmcpNavMore")
    page.wait_for_selector("#tmcpNavPanel.open", timeout=5000)
    page.click('#tmcpNavPanel a[href="/dashboard/audit"]')
...
def test_the_current_page_is_marked_in_the_menu(page, server_url):
    ...
    for key in ("projects", "agents", "sessions", "nodes"):
        ...
        marked = page.evaluate(
            """() => {
              const a = document.querySelector('.tmcp-global-nav a[aria-current="page"]');
...
def test_on_a_phone_every_destination_is_still_reachable(page, server_url):
    ...
    row_visible = page.evaluate(
        "() => getComputedStyle(document.querySelector('.tmcp-nav-links')).display !== 'none'")
    assert row_visible is False, "the wide link row is still shown on a 390px screen"

    page.click("#tmcpNavMore")
    page.wait_for_selector("#tmcpNavPanel.open", timeout=5000)
    hrefs = page.evaluate(
        "() => [...document.querySelectorAll('#tmcpNavPanel a')].map(a => a.getAttribute('href'))")
    for destination in DESTINATIONS:
        assert destination.href in hrefs, f"{destination.key} unreachable on a phone"

    page.click('#tmcpNavPanel a[href="/dashboard/nodes"]')
    page.wait_for_url("**/dashboard/nodes", timeout=15000)
    assert _nav_box(page)["visible"]


def test_escape_closes_the_panel_and_returns_focus(page, server_url):
    page.set_viewport_size(LAPTOP)
    _goto(page, server_url, "/dashboard")
    page.click("#tmcpNavMore")
    page.wait_for_selector("#tmcpNavPanel.open", timeout=5000)
    ...
    assert page.evaluate(
        "() => !document.querySelector('#tmcpNavPanel').classList.contains('open')")
    assert page.evaluate("() => document.activeElement.id") == "tmcpNavMore"


def test_the_menu_is_tab_reachable(page, server_url):
    """A keyboard user must reach the first destination without a mouse."""
    page.set_viewport_size(LAPTOP)
    ...
    for _ in range(6):
        ...
        reached = page.evaluate(
            ...
            " return a && a.closest && a.closest('.tmcp-global-nav') ? a.getAttribute('href') || a.id : null; }")
        if reached:
            break
