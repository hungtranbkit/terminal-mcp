...
# Both stores share the complete queue migration ladder.
LATEST_QUEUE_VERSION = max(m.version for m in QUEUE_MIGRATIONS)
...
def test_migration_lands_harness_tables_in_the_queue_database(tmp_path):
    ...
    assert connection.execute("PRAGMA user_version").fetchone()[0] == LATEST_QUEUE_VERSION
...
def test_migrating_twice_is_a_no_op(tmp_path):
    ...
    assert before == after == LATEST_QUEUE_VERSION
