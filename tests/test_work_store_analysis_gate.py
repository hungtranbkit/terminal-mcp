...
from terminal_mcp.work_analysis_gate import AnalysisGateError
...
def test_analysis_refusal_is_a_work_error_for_coordinator_and_control(store):
    from terminal_mcp.work_service import WorkService

    run = _run(store, _complete_gate(problem_statement=""))
    store.transition_run(run.work_id, ws.PLANNING)
    with pytest.raises(ws.WorkError) as excinfo:
        store.transition_run(run.work_id, ws.READY)
    assert isinstance(excinfo.value, AnalysisGateError)
    store.transition_run(run.work_id, ws.BLOCKED)
    result = WorkService(store).control(run.work_id, "resume")
    assert result["error"] == "INVALID_TRANSITION"
    assert "problem_statement" in result["detail"]
    assert store.get_run(run.work_id).state == ws.BLOCKED


def test_plan_reports_gate_refusal_without_claiming_ready(store, tmp_path):
    ...
    run = _run(store, _complete_gate(problem_statement=""))
    result = service.plan(run.work_id, [{"prompt": "implement change"}])
    assert result["error"] == "ANALYSIS_GATE_REFUSED"
    assert "problem_statement" in result["analysis_gate"]["missing_fields"]
    assert store.get_run(run.work_id).state == ws.PLANNING
    assert "planned" not in _events(store, run.work_id)
