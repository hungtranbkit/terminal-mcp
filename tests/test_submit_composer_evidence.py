...
def test_a_real_submit_clears_the_composer_and_confirms(tmux_session_factory, tmp_path):
    ...
    assert result["evidence"] == ["ADAPTER_ACK", "COMPOSER_RELEASED"]
...
def test_remote_node_path_shares_the_identical_submit_semantics(tmux_session_factory, tmp_path):
    ...
    assert sent["evidence"] == ["ADAPTER_ACK", "COMPOSER_RELEASED"]
    assert _enter_keystrokes(keylog) == 1
...
def test_windows_conpty_backend_degrades_to_unknown_never_to_not_activated(tmp_path):
    ...
    assert result["delivery_state"] in {"DELIVERY_UNKNOWN", "SUBMIT_STALLED"}, result
    assert result["delivery_state"] != "NOT_ACTIVATED"
    assert backend.sent_keys.count(["Enter"]) == 1, "exactly one Enter must still be delivered"
...
@pytest.mark.parametrize("colour", ["38;5;2", "38;2;12;34;2", "48;2;2;34;56"])
def test_ansi_colour_parameters_are_not_dim_attributes(colour):
    row = f"\x1b[{colour}m❯ real draft\x1b[0m"
    snapshot = read_composer([row], attributes_available=True)
    assert snapshot.state == COMPOSER_DRAFT
    assert snapshot.draft_text == "real draft"


def test_plain_unicode_api_and_ansi_reader_agree_on_unicode_spaces():
    from terminal_mcp import composer
    pane = ["─" * 78, "❯\u202factual draft", "─" * 78]
    assert composer.extract(pane) == "actual draft"
    assert read_composer(pane, attributes_available=True).draft_text == "actual draft"
