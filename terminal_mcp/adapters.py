...
class SubmitPolicy:
    ...
    uses_composer_evidence
        Consult composer.read_composer (an ANSI capture) as additional
        negative evidence: an agent CLI with a real prompt composer can be
...
CODEX_SUBMIT_POLICY = SubmitPolicy(
    uses_composer_evidence=True, require_draft_before_enter=True,
    max_enter_attempts=6, allow_enter_retry=True, allow_escape_recovery=True,
)
...
class GenericShellAdapter(AgentAdapter):
    ...
    def submit_ack_evidence(self, before: list[str], after: list[str], sent_text: str) -> bool:
        # This fallback also handles unknown raw-mode programs. Echoed input
        # alone cannot prove that they accepted Enter. Known shells use the
        # identity-guarded Enter-delivery path in core, where the actual
        # foreground command is available.
        return after != before
