...
* **Composer transition is the primary submit evidence.** A draft
  *leaving* the composer is direct, causal proof of acceptance; a pane
  diff is not. `OUTPUT_CHANGED` alone is never accepted as an ACK for a
  composer-based agent, and a composer that positively still holds this
  attempt's draft now **vetoes** a confirmation the older heuristics
  would have granted.
* **Per-agent `SubmitPolicy` (`adapters.py`).** "Claude and unknown
  agents are single-submit" is now a structural invariant of the adapter
  rather than four `adapter.name == "codex"` string checks in `core.py`.
  Config can narrow an agent's Enter budget, never widen it past what its
  policy permits. Codex's bounded, evidence-gated retry and its
  Escape+Enter recovery are unchanged.
