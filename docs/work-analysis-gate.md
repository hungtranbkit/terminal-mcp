> This document describes the opt-in WorkRun `metadata.analysis_gate` contract.
> Queue task analysis profiles are documented separately in [AI_ANALYSIS_GATE.md](AI_ANALYSIS_GATE.md).
...
Evaluator: `terminal_mcp/work_analysis_gate.py`. Enforcement point:
